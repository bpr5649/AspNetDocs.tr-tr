...
**Şekil 1**: klasöre yenı bir SQL veritabanı ekleme `SecurityTutorials.mdf` `App_Data` ([tam boyutlu görüntüyü görüntülemek için tıklatın](creating-the-membership-schema-in-sql-server-vb/_static/image3.png))
...
**Şekil 4**: veritabanını iliştirme `SecurityTutorials.mdf` ([tam boyutlu görüntüyü görüntülemek için tıklayın](creating-the-membership-schema-in-sql-server-vb/_static/image12.png))
...
**Şekil 6**: veritabanını olarak yeniden adlandırma `SecurityTutorialsDatabase` ([tam boyutlu görüntüyü görüntülemek için tıklayın](creating-the-membership-schema-in-sql-server-vb/_static/image18.png))
...
[![Üyelik şeması eklemek için ASP.NET SQL Server Kurulum Sihirbazı 'Nı kullanın](creating-the-membership-schema-in-sql-server-vb/_static/image20.png)](creating-the-membership-schema-in-sql-server-vb/_static/image19.png)
...
`Providers`Özelliğin içeriği, Web uygulamasının yapılandırma ayarları aracılığıyla belirtilir. Varsayılan olarak, tüm Web uygulamalarının türünde adlı bir sağlayıcısı vardır `AspNetSqlMembershipProvider` `SqlMembershipProvider` . Bu varsayılan üyelik sağlayıcısı ' de kaydedilir `machine.config` (konumunda bulunur `%WINDIR%\Microsoft.Net\Framework\v2.0.50727\CONFIG` ):
...
>  [Sonraki](creating-user-accounts-vb.md)
